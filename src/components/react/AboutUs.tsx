    ...
    >
      <div className="pt-12 sm:pt-16 md:pt-20 bg-[var(--color-primary)] text-white">
        <div className="container mx-auto px-4 text-center">
          <h2 className={`text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-6 md:mb-8 px-2 ${isVisible ? 'animate-fade-up animate-duration-700' : 'opacity-0'
            }`}>{title}</h2>
          <p className={`text-base sm:text-lg md:text-xl max-w-4xl mx-auto mb-8 md:mb-12 leading-relaxed px-2 ${isVisible ? 'animate-fade-up animate-duration-700 animate-delay-200' : 'opacity-0'
            }`}>
            {description}
          </p>

          {/* Decorative Icon */}
          <div className={`flex justify-center mt-6 md:mt-8 ${isVisible ? 'animate-fade animate-duration-1000 animate-delay-600' : 'opacity-0'
            }`}>
